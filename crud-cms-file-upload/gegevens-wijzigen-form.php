...
<body class="web-backend-inleiding">
	...
	<?php if($dashboard): ?>
		...
		<p><?= ( isset( $_SESSION['notification']['type'] ) ) ? $_SESSION['notification']['message'] : '' ?></p>

		<form action="gegevens-bewerken.php" method="POST" enctype="multipart/form-data">
		<ul>
			<li><label for="profielfoto">Profielfoto</label><img src="<?= ( isset( $fetchAssoc[0]['profile_picture'] ) ) ? 'img/'.$fetchAssoc[0]['profile_picture'] : 'img/placeholder.jpg' ?>" alt="<?= $saltUser[0] ?>"></li>
			<li><input  class="formArtikels" type="file" name="profielfoto" value="upload"></li>
			<li><label for="email">e-mail</label><input class="formArtikels" type="email" name="email" value="<?= $saltUser[0] ?>"></li>
			<li><input type="submit" name="toevoegen" value="gegevens wijzigen"></li>
		</ul>
		</form>
